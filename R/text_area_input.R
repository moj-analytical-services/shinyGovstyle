...
text_area_Input <- function(inputId, label, hint_label=NULL, row_no=5, error = FALSE, error_message = NULL, word_limit=NULL){
  govTextarea <- shiny::tags$div(class="govuk-form-group govuk-character-count", id=paste0(inputId,"div"),
    ...
    if (!is.null(word_limit)){
      shiny::tags$span(id=paste0(inputId,"wc"), class="govuk-hint govuk-character-count__message",
